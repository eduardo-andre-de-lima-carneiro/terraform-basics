...
- Authentication uses OIDC, a service connection, or a scoped token, not a long-lived cloud key in the repository. **Without it:** a leaked key keeps working until someone notices and rotates it by hand, and it usually grants more than the pipeline actually needs.
- State is stored in a locking remote backend or platform workspace, never in version control. **Without locking:** two runs writing state at the same time can corrupt it or silently apply half of a stale plan.
...
- Provider and module versions are pinned, and the `.terraform.lock.hcl` dependency lock file is committed. **Without the lock file:** the same configuration can resolve a different provider version on the next run and apply changes nobody wrote.
