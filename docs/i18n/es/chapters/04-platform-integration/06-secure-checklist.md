...
- La autenticación usa OIDC, una service connection o un token con alcance limitado, no una clave de nube de larga duración en el repositorio.
- El state se almacena en un backend remoto con bloqueo o en un workspace de la plataforma, nunca en el control de versiones.
...
- Las versiones de providers y módulos están fijadas, y el archivo de bloqueo de dependencias `.terraform.lock.hcl` está versionado.
